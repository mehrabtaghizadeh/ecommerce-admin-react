function OrderTable({orders}) {
      ...
      <tbody>
          ...
          <tr key={order._id} className="border-b-1 border-gary text-darkblue hover:bg-gary">
              <th scope="row" className="px-6 py-4 flex justify-start items-center gap-2 font-medium whitespace-nowrap text-darkblue">
                <div className='flex gap-2'>
                {order.lineItems.map(lineItem => 
                <div className='flex flex-col'>
                  <p>{lineItem.quantity}*{lineItem.title}</p>
                  {lineItem.color ? 
                  <p className='flex justify-center p-1 mt-2 bg-blue rounded text-white'>{lineItem.color}</p>
                  : ""}
                </div>
                  )}
                </div>
